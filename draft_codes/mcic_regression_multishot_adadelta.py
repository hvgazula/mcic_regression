...
run_index = '99_adadelta'
folder_name = '_'.join(('Part', str(run_index).zfill(2)))
...
for voxel in range(1):
    ...
    while not gottol(grad_remote, tol):
        ...
        Eg2 = rho * Eg2 + (1 - rho) * grad_remote * grad_remote
        dW = -np.sqrt(EdW2 + eps) / np.sqrt(Eg2 + eps) * grad_remote
        EdW2 = rho * EdW2 + (1 - rho) * dW * dW
        wc = wp - dW

        print(prev_obj_remote, curr_obj_remote, np.sum(np.square(grad_remote)))
    ...
    if curr_obj_remote != prev_obj_remote or np.sum(
            np.square(grad_remote)) > tol:
        flag = 1

#    with open('adadelta_output', 'a') as fh:
#    print(
#            '{:07d} {:^15d} {:^20.6f} {:^20.6f} {:^15.5f} {:^4d} \n'.
#            format(voxel, count, prev_obj_remote, curr_obj_remote,
#                   np.sum(np.square(grad_remote)), flag))
    print('Number of iterations for voxel {} is {}'.format(voxel, count))
    avg_beta_vector = wc
...
results = shelve.open(join(folder_name, 'decentralized_results'))
