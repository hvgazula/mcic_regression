...
def objtol(val1, val2, tol=1e-5):
    """Check if the gradient meets the tolerances"""
    return np.abs((val1 - val2)/val1) <= tol
...
#    wp = np.zeros(X1.shape[1])
    ...
    while not gottol(grad_remote, tol):
        ...
        historical_grad += np.square(grad_remote)
        adjusted_grad_denom = (fudge_factor + np.sqrt(historical_grad))
        adjusted_grad = grad_remote/adjusted_grad_denom

        wc = wp - eta * grad_remote

        print('{:07d} {:^15d} {:^20.6f} {:^20.6f} {:^15.5f} {:^15.7f}'.
          format(voxel, count, prev_obj_remote, curr_obj_remote, eta, np.sum(np.square(grad_remote))))
